package tech.mobisuite.faceplace.Detector;
...
public class FaceDetector extends Detector<Face> {
    ...
    public SparseArray<Face> detect(Frame frame) {
        ...
        final int kMinDimension = 147;      //min width dimension reqd
        final int kDimensionLower = 640;    //min height dimension reqd
        int width = frame.getMetadata().getWidth();     //width of frame
        int height = frame.getMetadata().getHeight();   //height of frame
        ...
        if (height > (2 * kDimensionLower)) {
            ...
            if (lowerWidth < kMinDimension) {
                ...
                frame = FramewithPaddingRight(frame, newWidth);
            }
            ...
            if (lowerHeight < kMinDimension) {
                ...
                frame = FramewithPaddingBottom(frame, newHeight);
            }
        } else if (width < kMinDimension) {
            frame = FramewithPaddingRight(frame, kMinDimension);
        }
     ...
     */
    private Frame FramewithPaddingRight(Frame originalFrame, int newWidth) {
     ...
     * increase the size to avoid the bug in the underlying face detector.
     */
    private Frame FramewithPaddingBottom(Frame originalFrame, int newHeight) {
        Frame.Metadata metadata = originalFrame.getMetadata();
        int width = metadata.getWidth();
        int height = metadata.getHeight();
